...
def initialize_session_state(config, db):
    ...
    if db is not None:
        ...
        if (
            "bots" not in st.session_state
            or "bot_sessions" not in st.session_state
            or refresh_needed
        ):
            st.session_state.bots = initialize_bots(db, config.get("bots", []))
            st.session_state.bot_sessions = fetch_user_sessions(
    ...
    if "current_session" not in st.session_state:
        st.session_state.current_session = create_new_session(
            st.session_state["name"], init_bot["id"]
        )
...
if not config:
    st.error("配置文件加载失败，请检查 CONFIG_FILE 环境变量。")
    st.stop()
...
if st.session_state["authentication_status"]:
    ...
    if os.getenv("MONGO_URI"):
        ...
        logger.warning(
            "MONGO_URI environment variable is not set. User sessions will not be fetched from the database."
        )
        ...
        if st.session_state.current_page == "chat":
            ...
            for date in sorted(grouped_sessions.keys(), reverse=True):
                ...
                for session in sorted(
                    grouped_sessions[date], key=lambda s: s["create_time"], reverse=True
                ):
                    st.button(
                        str(session["name"]),
                        key=session["id"],
            ...
            if db is not None:
                if st.button("加载更多", use_container_width=True):
                    ...
                    additional_sessions = fetch_user_sessions(
                        db, st.session_state["name"], skip=skip
                    )
